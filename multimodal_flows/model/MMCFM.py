...
class MultiModalFlowMatching(L.LightningModule):
    def __init__(self, config):
        ...
        self.bridge_discrete = UniformFlow(config.sigma)        
        self.save_hyperparameters(vars(config))
        self.config = config

        # time-dependent loss uncertainty weights
        self.uncertainty_net = MLP(config.n_embd, config.n_embd, n_out=3) 
    ...
    def loss(self, batch: DataCoupling) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        mse_1 = F.mse_loss(vt, targets_continuous, reduction="none")  # (B,D,dim_cont)
        mse_1 = (mse * state.mask).sum(dim=[1,2])                     # (B,)
        loss_mse_1 = mse_1 / state.mask.sum(dim=[1,2]).clamp_min(1.0)        # (B,)

        # MSE for discrete onehot states
        targets_discrete = self.bridge_discrete.conditional_drift(state, batch)  # (B,D,dim_cont)
        mse_2 = F.mse_loss(logits, targets_discrete, reduction="none")  # (B,D,dim_cont)
        mse_2 = (mse_2 * state.mask).sum(dim=[1,2])                     # (B,)
        loss_mse_2 = mse_2 / state.mask.sum(dim=[1,2]).clamp_min(1.0)        # (B,)
        ...
        u_1, u_2 = self.uncertainty_net(t_emb).unbind(-1)  # each (B,)
        w_1 = torch.exp(-u_1)
        w_2 = torch.exp(-u_2)
    
        # weighted loss:
        loss  = 0.5 * (w_1 * loss_mse_1 + u_1) + 0.5 * (w_2 * loss_mse_2 + u_2)       # (B,)

        return loss.mean(), loss_mse_1.mean(), loss_mse_2.mean(), w_1.mean(), w_2.mean() 
